...
SelectBox.prototype.addEvent = function() {
    ...
    this.allowInput && l.focus(function() {
        ...
        var g = f.val().replace(' ', '');
        //搜索过滤器
        var myFilter = function(value, text) {
            var result;
            if (escape(value).indexOf("%u") != -1) { //汉字
                result = text.indexOf(value) > -1;
            } else {
                var len = value.length
                result = ConvertPinyin(text).substring(0, len) === value || makePy(text)[0].toLowerCase().substring(0, len) === value || text.toLowerCase().indexOf(value) > -1;
            }
            if (result == true) {
                return false;
            } else {
        ...
        f.timer = setInterval(function() {
            ...
            if (c != g) {
                ...
                if (c == '') {
                    ...
                    k.children().hide();
                    //现在
                    $.each(k.children(), function (index, element) {
                        var othis = $(this)
                            ,text = othis.text()
                            ,not = myFilter(g, text);
                        if (!not){
