...
use app\common\model\UploadFileGroup as GroupModel;
use think\Model;
use think\Image;
use think\File;
...
//        print_r($file);exit();
        ...
        // 上传附件路径
        $_upload_path = ROOT_PATH . 'public/upload' . DS . $group . DS . $type . DS.date('Ymd').DS;
        // 附件访问路径
        $_file_path = ROOT_DIR.'upload/'.$group.'/'.$type.'/'.date('Ymd').'/';
        // 如果文件已经存在，直接返回数据
...
//        $upfile = $file->move($_upload_path,false);//保留原文件名
        ...
        runhook('system_file_upload', $data);
