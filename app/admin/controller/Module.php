...
class Module extends Admin
{
    ...
    public function index($status = 2)
    {
        ...
        if ($status == 0) {
            ...
            $sys_dir = config('tb_system.modules');
    ...
    public function install($id = 0)
    {
        ...
        if ($this->request->isPost()) {
            ...
            foreach ($info['tables'] as $t) {
                if (in_array($t, config('tb_system.tables'))) {
            ...
            if (file_exists($sql_file)) {
                ...
                if ($sql_list) {
                    ...
                    foreach ($sql_list as $v) {
                        // 过滤sql里面的系统表
                        foreach (config('tb_system.tables') as $t) {
    ...
    public function icon()
    {
        ...
        $image = get__ADMIN_JS__/home/ize($image_path.$module['name'] . '.png');
