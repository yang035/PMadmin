...
<div class="layui-form">
    <table class="layui-table mt10" lay-even="" lay-skin="row">
        ...
        <tr>
            <td><input type="checkbox" name="id[]" class="layui-checkbox checkbox-ids" value="{$key}" lay-skin="primary"></td>
            <td>{$i}</td>
            <td class="font12">
                <strong><a class="mcolor" href="#" onclick="zujian_user({$key})">{$vo['name']}</a></strong>
            </td>
                {eq name="vo['flag']" value="0"}
            <td class="font12 red"><a href="#" onclick="zujian_user({$key})" class="layui-btn layui-btn-xs layui-btn-warm">待处理</a></td>
                {else/}
            <td class="font12 green"><strong>&#10003</strong></td>
                {/eq}
        </tr>
        {/volist}
        </tbody>
    ...
    <form class="layui-form" action="{:url()}" method="post">
        ...
        <div class="layui-form-item">
            <div class="layui-input-block">
                ...
                <button type="submit" class="layui-btn layui-btn-normal normal_btn">推荐信(下一步)</button>
...
<script>
    ...
    function zujian_user(id) {
        var open_url = "{:url('Subject/addBaseUser')}?id="+id;
        if (open_url.indexOf('?') >= 0) {
            open_url += '&hisi_iframe=yes';
        } else {
            open_url += '?hisi_iframe=yes';
