...
<div>
    <div class="dtree" id="dtree_div" style="width: 500px;float: left">
        ...
        <script type="text/javascript">
            d = new dTree('d', false);
            var ajax_url = "{:url('admin/tool/getTreeDepartment')}";
                ...
                success:function(data){
                    ...
                    for(var i=0;i<data.length;i++){
                        if (typeof (data[i].role_id) == 'undefined'){
                            d.add(data[i].id,data[i].pid,'authority',data[i].id,data[i].name,false,false);
                        } else {
                            d.add(data[i].uid,data[i].department_id,'authority',data[i].id,data[i].nick+'['+data[i].realname+']',true,false);
                        }
    ...
    <script>
            ...
            $('.user_confirm').click(function () {
                ...
                if (i){
                    ...
                    window.parent.document.getElementById(m+'_id['+i+']').value = ','+ids;
                } else {
                    window.parent.document.getElementById(m+'_select_id').innerText=name;
                    window.parent.document.getElementById(m+'_id').value = ','+ids;
                }
