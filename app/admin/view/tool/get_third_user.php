...
<div>
    <div class="dtree" id="dtree_div" style="width: 500px;float: left">
        ...
        <script type="text/javascript">
            d = new dTree('d', true);
            var ajax_url = "{:url('admin/tool/getThirdUser')}";
            ...
            $.ajax({
                ...
                success:function(data){
                    //根目录
                    d.add(0,-1,"根目录");
                    d.add(1,0,'authority',1,'第三方用户',true,false);
                    for(var i=0;i<data.length;i++){
                        d.add(data[i].uid, 1, 'authority', data[i].id, data[i].username + '[' + data[i].name + ']', false, false);
                    }
                    document.write(d);
                    d.openAll();
                }
