...
<script>
    ...
    layui.use(['jquery', 'laydate','element', 'upload','form'], function() {
        ...
        try{
            ...
            $('.show_div').hide();
            var h = "出错！必须满足以下要求：<br>一、必须用 IE 浏览器（WIN10系统 Edge要转换IE浏览器）；<br>二、设置步骤：IE菜单 -> 工具 -> Internet选项 -> 安全 -> 自定义级别 -> 对未标记为可安全执行脚本的ActiveX控件初始化并执行脚本--选‘提示’，保存修改项；<br>三、刷新浏览器，若有弹出框选择‘是或确认’；<br>四、等待内容自动填充后，点击‘提交’按钮即可。";
            $('.hide_div').html(h);
        }
