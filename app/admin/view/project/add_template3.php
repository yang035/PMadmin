...
layui.use(['jquery', 'laydate','upload','form','element'], function() {
    ...
    select_union('{$p_data.subject_id}','{$p_data.major_cat}');

        form.on('select(rid)', function(data){
            var num = data.elem.id.substring(4);
            select_union('{$p_data.subject_id}','{$p_data.major_cat}',data.value,0,1,num);
        });
    ...
    $(".new_task").click(function(){
        ...
        select_union('{$p_data.subject_id}','{$p_data.major_cat}',0,0,0,idNum);
        form.on('select(rid'+idNum+')', function(data){
            select_union('{$p_data.subject_id}','{$p_data.major_cat}',data.value,0,1,idNum);
        });
        element.render();
        form.render();
    });
...
function checkScore(i,v){
    var t_s={$p_data.score-$p_data.real_score},id={$p_data.subject_id},major_cat={$p_data.major_cat},major_item=$("#i_id"+i).val(),major_item_name=$("#i_id"+i).find("option:selected").text(),total = 0;
    if (major_item > 0){
        $.ajax({
            type: 'POST',
            url: "{:url('getSmallMajorScore')}",
            data: {id:id,major_cat:major_cat,major_item:major_item,t_s:t_s},
            dataType:  'json',
            success: function(data){
